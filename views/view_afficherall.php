...
if($allArticles->rowCount() > 0 ){
    while($row = $allArticles->fetch(PDO::FETCH_ASSOC)) {            
        ...
        $return_string .= '<a href="miscs/ajoutpanier.php?idart='.$row['idArticle'].'"><img class="img-fluid" src="assets/img/panier.jpg" width=36 height=36></a>';
        $return_string .= '<br></p>'."\n";
        $return_string .= '<p class="titrearticle">'.utf8_encode($row['nomArticle']).'</p>'  .  "\n";
        $return_string .= '<p><img class="img-fluid" src="'.$row['photoArticle'].'" alt="'. $row['nomArticle'] .'" width=360 height=360></p>'."\n";
